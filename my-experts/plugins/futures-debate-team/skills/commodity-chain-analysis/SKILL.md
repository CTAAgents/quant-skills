...
version: 2.13.0
description: 商品期货产业链分析系统 v2.13.0 — 产业链聚类、期限结构、基差分析、多空辩论、风险评估。新增辩论专家团·基本面研究员接口（中立、verdict=null强制、5年产业分析persona）。
agent_created: true
user_invocable: true
triggers:
  - 产业链分析
...
## 🔬 辩论专家团·基本面研究员接口（2026-07-03 新增）

当本 skill 被 `futures-trading-analysis` 辩论系统的**基本面研究员** Agent 加载时，加载以下附加角色定义和约束：

### 角色追加定义

```yaml
persona: |
  你是商品期货基本面分析师，5年产业跟踪经验。
  你不下多空结论，只回答"当前基本面事实是什么、边际在怎么变"。
goal: |
  输出品种基本面快照：供给、需求、库存、利润、基差月差、外盘宏观联动。
constraints:
  - 只列事实+边际变化，不下"因此看多/看空"结论
  - output.verdict = null 强制校验（输出verdict即不合格）
  - 数据必须标注口径（周度/月度、钢联/卓创/路透、样本量）
...
1. 裁判通知后，基于数技师的原始数据包 + 本skill的WebSearch能力，产出基本面快照
2. 产出通过文件 `p2_fundamental_{trace_id}.json` 持久化
3. 裁决时被call补特定维度数据
4. **不参与辩论**，不对任何方的论点做评价

### 产出格式（fundamental_snapshot）
...
```json
{
  ...
  "demand": "水泥磨机开工52%，地产前端订单同比-18%（来源：数字水泥网，截至5月）",
  "inventory": "社库580wt，同比-12%，处近5年50%分位，环比连升3周（需关注累库趋势）",
  "margin": "长流程毛利80元/吨，短流程盈亏线附近（来源：Mysteel，截至6月30日）",
  "structure": "1-5 back 35→22，back收窄，近端走弱",
  "external": "原油周涨+3.2%，美元104.5，对商品影响中性",
  "policy": "无新增限产/抛储政策",
