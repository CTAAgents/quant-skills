...
def download_kline_data_parallel(symbols: List[str], days: int = 756, use_cache: bool = True) -> Dict[str, pd.DataFrame]:
    """通过 futures-data-search MultiSourceAdapter 获取K线数据
    
    数据源由 futures-data-search 的 MultiSourceAdapter 统一调度，
    优先级：WH6本地(1.0) → 交易所API(0.98) → TqSdk(0.97) → 东方财富(0.95) → AKShare(0.85)
    """
    results = {}
    symbols_to_fetch = []
    ...
    logger.info(f"通过 MultiSourceAdapter 获取 {len(symbols_to_fetch)} 个品种 K线数据（{days}天历史）")
    
    # 加载 MultiSourceAdapter（futures-data-search 统一数据源调度）
    try:
        ...
        logger.error(f"无法获取数据，请确保 futures-data-search skill 已安装")
        return results
    
    # 通过适配器逐个获取品种
    for symbol_key in symbols_to_fetch:
        logger.info(f"  获取 {symbol_key}...")
        try:
            result = adapter.get_kline(variety=symbol_key, days=days)
            if result.get("success") and result.get("data"):
                raw_data = result["data"]
                data_source = result.get("data_source", "unknown")
                logger.info(f"    {symbol_key}: 数据源={data_source}, 置信度={result.get('confidence', 0)}")
                
                # 转换为DataFrame（与原有格式兼容）
                records = []
                ...
                if records:
                    ...
                    logger.info(f"  ✅ {symbol_key}: 获取 {len(df)} 行数据（数据源: {data_source}）")
                else:
                    logger.warning(f"  ⚠ {symbol_key}: 无有效K线记录")
            else:
                logger.warning(f"  ⚠ {symbol_key}: 数据获取失败 - {result.get('error', '未知错误')}")
        except Exception as e:
            logger.error(f"  ❌ {symbol_key}: 获取异常 - {e}")
    
    logger.info(f"采集完成，成功获取 {len(results)}/{len(symbols)} 个品种")
    return results
